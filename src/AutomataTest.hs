...
-- |>> (3) Check it reads the correct value for bottom left
-- |       in the first iteration of 1100 test pattern
-- |>> (4) Check it reads the correct value for (1,0)
-- |       in the first iteration of 1100 test pattern
...
getTests = 
 ...
 ,Test "get: Bottom L 1100" (assertEqual (get pattern1100 (10,2))
   ((Just Alive) :: Maybe QRCell))
 ,Test "get: (1,0) 1100" (assertEqual (get pattern1100 (1,0))
   ((Just Dead) :: Maybe QRCell))
 ,Test "get: (0,1) in 2x2" (assertEqual (get (Grid 2 2 [Alive, Alive, Dead
  ,Dead]) (0,1)) ((Just Dead) ::Maybe QRCell))
 ,Test "get: (0,3) in 2x2" (assertEqual (get (Grid 2 2 [Alive, Alive, Dead
...
-- |>> (2) Tests can produce specification example output
-- |>> (3) Tests a typical input to helper nPair
-- |>> (4) Tests a typical input to helper nList
-- |>> X (5) Tests allCoords 0 0 -> []
...
allCoordsTest = 
 ...
 -- ,Test "allCoords (-1) (-1)" (assertEqual (allCoords (-1) (-1)) 
 --   ([] :: [GridCoord]))]
...
-- ================== evolveQR Tests ================= --
-- | Checks that 1100 reaches the alternating pattern
-- | and that 1130 reaches steady state as specified
...
evolveQRTests = [Test "evolveQR: 1100 12 and 14 ==" (assertEqual 
 ...
 ,Test "evolveQR: 1100 13 and 15 ==" (assertEqual 
  (evolveQR 13 pattern1100) ((evolveQR 15 pattern1100) :: Grid QRCell))
...
renderQRTests = [Test "renderQR Alive" (assertEqual (renderQR: Alive)
  ((coloured blue  (solidRectangle 1 1))::Picture))
 ,Test "renderQR Dead" (assertEqual (renderQR: Dead) 
  ((coloured black  (rectangle 1 1))::Picture))]
-}